...
pub fn send_byte(cs: &CriticalSection) -> u8 {
    ...
    let byte = 0u8
        ...
        | (on_switch.is_high()                                      // Switch lights on or off
            || (auto_switch.is_high() && light_sensor.is_low())     // Even in auto mode
          as u8 ) << 2
